...
val cl = [s1, s2, s3, s4, s5, s6, s7, s8, s9]

val a2 = map rev cl
val crates = Vector.fromList a2

fun move (arr, count, fromThis, toThat) =
    if count > 0
    then let
	val from = fromThis - 1;
	val to = toThat - 1;
	...
	val a::atail = f
	val vec1 = Vector.update (arr, from, atail)
	val vec2 = Vector.update (vec1, to, a::t)
				 
    in
	move (vec2, count - 1, fromThis, toThat)
    end
    else arr
...
fun topCrates (arr) =
    let
	fun loop (pos, topCrates) =
	    if pos >= 0
	    then let val c = Vector.sub (arr, pos) in loop (pos-1, (hd c)::topCrates) end
	    else implode topCrates
    in
